function FBAsolution = optimizeCbModel_Ratio(model,osenseStr, minNorm, allowLoops,matConstr,senseConstr)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%SECTIONS HERE ARE CORRECTED TO ALLOW A FLUX CONSTRAINT RELATIONSHIP TO BE
%SET IN THE matConstr VARIABLE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if ~isfield(model,'csense')
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%% MODIFICATION FOR matConstr %%%
    LPproblem.csense((length(LPproblem.csense)+1):length(LPproblem.csense)+length(senseConstr)) = senseConstr;
    %%% MODIFICATION FOR matConstr %%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if (~isfield(model,'b'))
    LPproblem.b = [zeros(size(model.S,1),1);zeros(size(matConstr,1),1)];
else
    LPproblem.b = [model.b;zeros(size(matConstr,1),1)];
end

% Rest of the LP problem
LPproblem.A = [model.S;matConstr];
LPproblem.c = model.c;
...
%         solution = solveCobraQP(LPproblem);
        solution = solveCobraQP_custom(LPproblem); %hack to get 'max' QP to work
